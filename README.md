...
## Viewing digests in a Streamlit app

`streamlit_app.py` is a read-only viewer — it displays whatever digests
already exist in `digests/`. **It makes zero Anthropic API calls and
costs nothing to run or deploy**, since it only reads files the monthly
GitHub Actions run already produced.

### Deploying it free (Streamlit Community Cloud)

1. Go to [share.streamlit.io](https://share.streamlit.io), sign in with
   your GitHub account.
2. Click **New app**.
3. Pick your `pharma-trend-agent` repo, branch `main`, and set
   **Main file path** to `streamlit_app.py`.
4. Click **Deploy**. No secrets needed — this app doesn't touch the
   Anthropic API at all.
5. You'll get a public URL (or you can restrict access in the app
   settings) that shows the latest digest, with a month picker, topic
   filter, and search box.

Streamlit Community Cloud automatically redeploys when new commits land
on the repo — so next month, when the GitHub Actions workflow commits a
new digest, the viewer picks it up on its own within a few minutes, no
...
Opens at `http://localhost:8501`. Same zero-cost guarantee — it's just
reading local files.
...
3. That's it — the workflow is set to **manual-only** (no automatic
   schedule). Whenever you want a new digest, go to the **Actions** tab
   → **Monthly Pharma Trend Digest** → **Run workflow**. Each month's
   digest lands in the `digests/` folder in the repo.
...
- **Turn automatic monthly runs back on**: uncomment the `schedule`
  block in `.github/workflows/weekly-digest.yml` ([crontab.guru](https://crontab.guru)
  helps with cron syntax if you want a different cadence).
